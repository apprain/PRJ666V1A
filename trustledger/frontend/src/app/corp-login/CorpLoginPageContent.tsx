...
export default function CorpLoginPageContent() {
  ...
  const redirectUrl = searchParams.get("redirect") || "/organization/dashboard";
  const serviceUrl =
  ...
  async function handleLogin(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${serviceUrl}/auth/corp-login`, {
        method: "POST",
      ...
      localStorage.setItem("accountType", "corporate");
      ...
      <section className="relative z-10 mx-auto grid min-h-[calc(100vh-77px)] max-w-7xl items-center gap-10 px-5 py-10 sm:px-8 lg:grid-cols-[1fr_480px] lg:py-14">
        <div className="hidden max-w-xl lg:block">
          ...
          <p className="mt-6 text-lg leading-8 text-slate-600">
            Review statements securely shared with your organization, verify
            access details, and download authorized documents.
          </p>

          <div className="mt-9 space-y-5">
            {[
              "Organization-bound statement access",
              "Secure QR and share-link verification",
              "Controlled and auditable downloads",
            ].map((item) => (
        ...
        <div className="w-full rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_24px_65px_rgba(16,24,40,0.10)] sm:p-9">
          <div className="mb-8">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-500">
              ...
              securely shared with your institution.
            </p>
          </div>
          ...
          <div className="mt-7 flex items-center justify-center gap-2 rounded-xl bg-[#f1fbf3] px-4 py-3 text-xs text-slate-600">
            ...
            Organization access is authenticated and securely processed.
          </div>
